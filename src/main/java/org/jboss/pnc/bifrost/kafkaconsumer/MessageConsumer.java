...
public class MessageConsumer {
    ...
    @Transactional
    @WithSpan()
    public void consume(@SpanAttribute(value = "json") List<String> jsonBatchValues) {

        logger.info("Consuming {} json lines", jsonBatchValues.size());

        for (String json : jsonBatchValues) {
            processJson(json);
        }
    }
